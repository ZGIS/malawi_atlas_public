      ...
      },
      {
        ...
        "groups": [{
          ...
          },
          {
            "groupName": "Nsanje",
            "layers": [{
                "title": "Wind hazards (Nsanje) – PGIS (2013)",
        ...
        "groups": [

          {
            ...
            "layers": [{
                ...
                "parentGroup": "Salima Studies",
                "group": "Flood vulnerability index (chart)",
                "title": "Piecharts",
...
function createWMSLayerFromJSON(layerJSON) {
  ...
  var legend = 'http://isi.zgis.at/geoserver/wms?' +
    ...
    '&FORMAT=image/gif&LAYER=malawi:' +
    layerJSON.name;

  return new ol.layer.Tile({
    title: layerJSON.title, // use title instead of name, because of consistency
    name: layerJSON.title, // only for display in layer tree
    lid: layerJSON.name, // layer ID
    group: layerJSON.group,
    parentGroup: layerJSON.parentGroup,
    visible: false,
    opacity: layerJSON.opacity,
    dataqtip: layerJSON.dataqtip,
    thematicGroup: layerJSON.thematicGroup,
    legend: legend,
    legendHeight: legendHeight,
    description: layerJSON.description,
    field_aliases: layerJSON.field_aliases,
    feature_info_activated: layerJSON.feature_info_activated,
    source: new ol.source.TileWMS({
      url: 'http://isi.zgis.at/geoserver/malawi/wms',
      params: {
